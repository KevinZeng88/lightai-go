...
POST   /api/run-templates/{id}/render-preview
```
...
`POST /api/model-deployments` 创建部署时不自动启动。`desired_state` 创建时默认为 `stopped`。
...
```bash
# 1. 登记模型
curl -X POST /api/model-artifacts -H 'Cookie: ...' \
...
# 2. 登记运行环境
curl -X POST /api/runtime-environments -H 'Cookie: ...' \
...
# 3. 登记启动模板
curl -X POST /api/run-templates -H 'Cookie: ...' \
