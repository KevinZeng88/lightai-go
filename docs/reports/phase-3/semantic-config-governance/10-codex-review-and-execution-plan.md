...
| `deployment.served_model_name` | deployment_service | string | Deployment | Artifact name | non-empty when adapter requires; format safe | differs from artifact name | adapter emits served-name flag/env when supported | common | `backend.common.served_model_name`, raw `served_model_name` |
...
| `model_runtime.max_model_len` | model_runtime | integer | Deployment | `model_runtime.context_length` / ModelArtifact facts / backend recommendation | positive integer | above model context length; estimated VRAM risk | vLLM `--max-model-len`, SGLang `--context-length`, llama.cpp `--ctx-size` | Deployment common/advanced | `backend.arg.max_model_len`, raw `max_model_len`, `--max-model-len` |
...
`model_runtime.max_model_len` is owned by `model_runtime` even when copied to Deployment. Deployment can modify its copied snapshot value, but the semantic definition and validation rules remain model-runtime owned. `deployment.served_model_name` is owned by `deployment_service` because it names the served API surface, not the host-port exposure boundary.
...
| Acceptance | Plan answers target, current behavior, gap, reuse, migration, removal, batches, and finalized design decisions. |
...
| Item | Current code impact | Decision | Reason | Alternative / risk note |
| --- | --- | --- | --- | --- |
| Q1. `service_json` authority | `deployment_lifecycle_handlers.go`, `deployment_preview_handlers.go` and `DeploymentWizard.vue` currently read/write `service_json.host_port` and `service_json.container_port`; RunPlan start combines `service_json` with ConfigSet. | Semantic Deployment snapshot is the internal authority. `service_json` is derived response or transition storage only. | A second writable service object is the main source of drift between preview/create/patch/start. | If transition storage is kept, generate it from snapshot with checksum and reject direct writes when it differs. |
| Q2. `runtime.command` editability | Runtime handlers and catalog loader persist `launcher.command`; ConfigEdit taxonomy currently protects some deployment-layer command edits. | Ordinary users do not edit raw command. Normal UI uses `process_start_profile` or template selection; raw command is advanced/admin diagnostic tier. | Raw argv edits bypass adapter mapping and can invalidate service/health/model mount assumptions. | Advanced/admin edits require explicit warning and re-probe before deployment start. |
| Q3. `context_length` deployment editability | `ModelArtifactsPage.vue` and artifact APIs store context length facts; RunPlan/raw params also use `max_model_len` and sometimes `context_length`. | `context_length` is model fact/recommendation source. Deployment edits `model_runtime.max_model_len`, not `model_runtime.context_length`. | The model's physical/context fact should not become a mutable deployment runtime knob. | If a local exception is required, introduce `model_runtime.context_length_override` and keep scanner fact immutable. |
| Q4. Adapter mapping UI | `BackendsPage.vue` currently allows Add Parameter with `backend.arg.*`, which mixes catalog authoring and runtime values. | Adapter mapping is catalog/dev-file only in this phase, not ordinary UI. | Mapping is backend-authoring metadata; exposing it as normal UI will recreate raw CLI key leakage. | A future admin/dev mapping editor may write semantic key + mapping, never `backend.arg.*` values. |
| Q5. GPU binding ownership | Deployment uses `placement_json`; Docker options contain `devices` and `optional_devices`; RunPlan also needs visible devices. | `scheduler.accelerator_ids` stays in placement/scheduler layer; Docker device mapping belongs to `docker.*` semantic config. | GPU allocation and Docker device mount are related but have different owners and validation. | ConfigEditView may show placement summary as diagnostic/read-only while actual binding remains in placement editor. |
...
- Design decisions finalized for Batch 1 start.
- No unresolved design issue blocks Batch 1.
- Remaining implementation risks are documented as alternatives/risk notes.
