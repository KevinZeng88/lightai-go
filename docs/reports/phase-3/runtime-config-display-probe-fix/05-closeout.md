...
## Batch 4: Runtime Config UI Regression Closeout (`b4a1498` + `<latest>`)

### Root Causes

| Issue | Root Cause |
|-------|-----------|
| CUDA_VISIBLE_DEVICES in user-editable env | 10 catalog runtime YAMLs included `CUDA_VISIBLE_DEVICES: "{{vendor_visible_devices}}"` (or ASCEND equivalent) in their `env:` section. This was materialized into `runtime.env` ConfigSet item and displayed as user-editable default environment variable. Per design, vendor device variables must only be generated by the RunPlan device-binding resolver at deployment time. |
| runtime_type chain verification | Preflight already hardcodes `RuntimeType: "docker"` (commit `ee35b5b`). Catalog seed stores `runtime_type` from YAML `runner_type: docker`. Test confirms full chain: catalog → backend_runtime → NBR → all resolve `runtime_type=docker`. |

### Changes

| File | Change |
|------|--------|
| `configs/backend-catalog/runtimes/vllm/nvidia-docker.yaml` | Remove `env: CUDA_VISIBLE_DEVICES` |
| `configs/backend-catalog/runtimes/vllm/metax-docker.yaml` | Remove `env: CUDA_VISIBLE_DEVICES` |
| `configs/backend-catalog/runtimes/vllm/huawei-docker.yaml` | Remove `env: ASCEND_VISIBLE_DEVICES` |
| `configs/backend-catalog/runtimes/sglang/nvidia-docker.yaml` | Remove `env: CUDA_VISIBLE_DEVICES` |
| `configs/backend-catalog/runtimes/sglang/metax-docker.yaml` | Remove `env: CUDA_VISIBLE_DEVICES` |
| `configs/backend-catalog/runtimes/sglang/huawei-docker.yaml` | Remove `env: ASCEND_VISIBLE_DEVICES` |
| `configs/backend-catalog/runtimes/llamacpp/nvidia-docker.yaml` | Remove `env: CUDA_VISIBLE_DEVICES` |
| `configs/backend-catalog/runtimes/llamacpp/metax-docker.yaml` | Remove `env: CUDA_VISIBLE_DEVICES` |
| `configs/backend-catalog/runtimes/llamacpp/huawei-docker.yaml` | Remove `env: ASCEND_VISIBLE_DEVICES` |
| `configs/backend-catalog/runtimes/ollama/nvidia-docker.yaml` | Remove `env: CUDA_VISIBLE_DEVICES` |
| `internal/server/api/runtime_boundary_test.go` | Add `TestCatalogEnvDoesNotIncludeVendorDevicePlaceholder` and `TestPreflightResolvesRuntimeTypeDocker` |
| `internal/server/api/metax_device_binding_test.go` | Update NVIDIA/MetaX env assertions for new design (vendor device vars generated by RunPlan, not catalog) |

### New Tests

- `TestCatalogEnvDoesNotIncludeVendorDevicePlaceholder` — confirms 0 NVIDIA runtimes have CUDA_VISIBLE_DEVICES in runtime.env after seed ✅
- `TestPreflightResolvesRuntimeTypeDocker` — confirms vLLM Docker chain: template `runtime_type=docker`, clone preserves it ✅

### Runtime-Type Chain Verification

```
catalog YAML (runner_type: docker)
  → SeedCatalog → backend_runtimes.runtime_type = 'docker'
  → Clone → copy preserves runtime_type = 'docker'
  → Enable NBR → node_backend_runtimes (inherits from template)
  → preflightDeployment → RuntimeType hardcoded to "docker" (line 943)
  → Resolve() → passes validation (line 216-220)
```

Confirmed: NO path loses `runtime_type`. Preflight hardcodes "docker", catalog seed stores it correctly.
