...
| Operational | Docker model serving full correlation | RC3-E2E | Deployment→Instance→Task→Operation→Agent→Node→Container→Lease chain | 8 IDs correlated end-to-end; task lifecycle: created→claimed→container→health→completed (32s) | /tmp/lightai-go-rc3-e2e/docker-model-serving/logs/operation-correlation.md | Fixed |
| Operational | Audit log table + tenant_id | REVIEW-009 | `SELECT COUNT(*), tenant_id FROM audit_logs` | audit_logs table exists with tenant_id (V12); 0 records — no writer | /tmp/lightai-go-rc3-e2e/docker-model-serving/logs/audit-logs-query-output.txt | DOCUMENTED_BLOCKER |
| Operational | Structured log traceability | RC3-LOG | request_id, user_id+tenant_id, agent_id+node_id on every entry | request_id on all API calls; login records user_id+tenant_id; heartbeat tagged with agent_id+node_id; operation_id on task lifecycle | /tmp/final-e2e-logs/server.log, /tmp/final-e2e-logs/agent.log | Fixed |
| Operational | /metrics INFO noise count | REVIEW-030 | Count INFO-level /metrics entries in agent log | 0 /metrics INFO entries; heartbeat/task_poll/gpu_metrics summarized every ~64s, not per-cycle | /tmp/lightai-go-rc3-e2e/docker-model-serving/logs/metrics-noise-count.txt | Fixed |
| Operational | Docker lifecycle with operation_id | RC3-DOCKER | docker.create/docker.start/health_check/task_result log chain | 9 log entries, all tagged with operation_id, container_id, instance_id | /tmp/lightai-go-rc3-e2e/agent-docker-evidence.log | Fixed |
| Operational | Lease reserve/activate/release visible | REVIEW-004 | `SELECT lease_owner, lease_expires_at FROM agent_tasks` | lease_owner=agent_id, lease_expires_at=claim+5min, generation+attempt tracking | /tmp/tmp.QNZOeMR1oP/data/lightai.db agent_tasks | Fixed |
| Closeout | git push to remote | GLOBAL | `git push origin phase-3-runtime-observability-closeout` | 0f20b8f..49fb7b8 pushed successfully | git push output | Fixed |
