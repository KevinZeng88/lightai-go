...
## 2026-06-18 Boundary Round: Backend Versions, Runtime Templates, Node Runtime Configs

| ID | Issue | Evidence | Impact | Status | Fix Location | Verification | Final Decision |
| -- | ----- | -------- | ------ | ------ | ------------ | ------------ | -------------- |
| MRW-BND-001 | BackendVersion defaults were not copied into BackendRuntime as a frozen source snapshot | BackendRuntime creation only stored backend_version_id and could still rely on live BackendVersion defaults during RunPlan preflight | BackendVersion edits could silently affect existing runtime templates | FIXED | `internal/server/db/db.go` V17, `internal/server/api/runtime_handlers.go`, `internal/server/api/deployment_lifecycle_handlers.go` | `TestCreateBackendRuntimeCopiesBackendVersionSnapshot`; `go test ./internal/server/api` PASS | Closed |
| MRW-BND-002 | BackendVersion user catalog lacked add/edit/sync API | Existing API exposed BackendVersion list only | Operators could not maintain user backend versions through managed catalog | FIXED | `internal/server/api/backend_handlers.go`, `internal/server/api/router.go`, `web/src/pages/BackendsPage.vue`, `web/src/api/backends.ts` | `TestBackendVersionCreatePatchAndReloadUserCatalog`; `npm --prefix web test -- --runInBand` PASS | Closed |
| MRW-BND-003 | Runtime template page still managed node runtime configs | `BackendRuntimesPage.vue` exposed add/check/delete node actions inside template detail | Users could infer BackendRuntime is node-bound | FIXED | `web/src/pages/BackendRuntimesPage.vue`, `docs/design/runtime-template-node-runtime-snapshot.md` | `npm --prefix web run build` PASS; code review confirms template page is read-only for node usage references | Closed |
| MRW-BND-004 | Runner config wizard did not clearly show selected Docker image | Docker image row click was highlight-only and the selected image was not shown in a stable field | Users could not see which image_ref would be saved | FIXED | `web/src/components/DockerImagePicker.vue`, `web/src/pages/RunnerConfigsPage.vue`, `web/src/locales/zh-CN.ts`, `web/src/locales/en-US.ts` | `npm --prefix web run build` PASS; i18n key test PASS | Closed |
| MRW-BND-005 | NodeBackendRuntime list lacked edit/check actions and snapshot edit invalidation coverage | RunnerConfigsPage only had detail/delete; PATCH did not expose snapshot editing in Web | Node runtime configs could not be maintained after creation | FIXED | `web/src/pages/RunnerConfigsPage.vue`, `internal/server/api/node_runtime_handlers.go`, `internal/server/api/runtime_handlers.go` | `TestPatchNodeBackendRuntimeSnapshotFieldsNeedRecheck`; `npm --prefix web run build` PASS | Closed |
| MRW-BND-006 | BackendRuntime list boundary needed regression coverage | Existing list aggregation was not explicitly tested against NodeBackendRuntime leakage | Future changes could mix templates and node configs again | FIXED | `internal/server/api/runtime_boundary_test.go` | `TestBackendRuntimeListShowsTemplatesWithNodeAggregatesOnly` PASS | Closed |
| MRW-BND-007 | BackendVersion catalog used old/runtime-shaped version rows | Existing seed contained `openai-latest`, old vLLM/SGLang versions, vendor-key image maps, and no explicit official baseline fields | BackendVersion could be confused with runtime/vendor configuration | FIXED | `internal/server/db/db.go`, `configs/backend-catalog/versions/**`, `internal/server/api/backend_handlers.go` | `TestBackendVersionCatalogIsSoftwareOnly`; `go test ./internal/server/api` PASS | Closed |
| MRW-BND-008 | System/user BackendVersion mutability rules needed explicit API coverage | System version PATCH behavior and clone/user edit behavior were not fully tested | System catalog could be accidentally overwritten | FIXED | `internal/server/api/backend_handlers.go`, `internal/server/api/runtime_boundary_test.go`, `web/src/pages/BackendsPage.vue` | `TestSystemBackendVersionReadOnlyAndCloneable`, `TestBackendVersionCreatePatchAndReloadUserCatalog`; Web build/test PASS | Closed |
| MRW-BND-009 | BackendVersion API wrote DB before user catalog file | Create/edit/clone path updated DB rows before writing YAML, so DB could become the catalog source of truth | Catalog export/import and reload semantics were ambiguous | FIXED | `internal/server/api/backend_handlers.go`, `cmd/server/main.go`, `internal/server/api/runtime_boundary_test.go` | `TestBackendCatalogReloadLoadsSystemAndUserFilesWithoutMutatingRuntimeSnapshots`; full verification PASS | Closed |
| MRW-BND-010 | E2E model root add step failed on repeated runs when the same root already existed | `scripts/e2e-model-runtime-wizard-nvidia-api.sh` failed at `add_model_root` with `model root already exists` | Real-machine E2E could fail for state left by a prior run | FIXED | `scripts/e2e-model-runtime-wizard-nvidia-api.sh` | Script now reuses an existing enabled root for the same path; E2E verification recorded in final run | Closed |

No problems from this boundary round are left only in chat history.
