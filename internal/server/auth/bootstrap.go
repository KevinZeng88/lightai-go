...
func InitBootstrap(database *db.DB, cfg BootstrapConfig) error {
	...
	if userCreated {
		...
		if autoGenerated {
			log.Info("bootstrap admin created with auto-generated password",
				"username", cfg.Username,
				"password", "[REDACTED - shown only once in console]",
			)
			// Print the password to stderr so it can be captured.
		...
		if err := writeInitialCredentials(credPath, cfg.Username, password); err != nil {
			...
			fmt.Fprintf(os.Stderr, "Initial credentials saved: %s\n", credPath)
		}
	} else {
