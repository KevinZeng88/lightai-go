...
func TestNVIDIADifferentiatesFromMetaX(t *testing.T) {
	...
	// NVIDIA should NOT have CUDA_VISIBLE_DEVICES in user-editable env.
	// Vendor device variables (CUDA_VISIBLE_DEVICES, ASCEND_VISIBLE_DEVICES)
	// are now generated by the RunPlan device-binding resolver at deployment
	// time, not stored as catalog runtime template defaults.
	nvEnv := extractEnvJSON(t, nv)
	for k := range nvEnv {
		if k == "CUDA_VISIBLE_DEVICES" {
			t.Error("NVIDIA runtime template should NOT have CUDA_VISIBLE_DEVICES in user-editable env — it is now generated by RunPlan resolver")
		}
	}

	// MetaX differentiates from NVIDIA via docker_options (privileged, devices,
	// MACA_ env vars in docker_options), not via runtime.env. The vendor device
	// variable has been removed from env so verify the template still carries
	// MetaX-specific docker options.
	mxEnv := extractEnvJSON(t, mx)
	for k := range mxEnv {
		if strings.HasPrefix(k, "CUDA_VISIBLE_DEVICES") || strings.HasPrefix(k, "ASCEND_VISIBLE_DEVICES") {
			t.Error("MetaX runtime template should NOT have vendor device placeholder in user-editable env")
		}
	}
	// MetaX still identified by docker_options (privileged, devices).
	_ = mxEnv
}
