...
func artifactCapabilitySet(req map[string]interface{}) map[string]interface{} {
	...
	if v, ok := req["capability_sources"]; ok {
		if m, ok := v.(map[string]interface{}); ok {
			sources = m
		}
	}
	parameterDefaults := []interface{}{}
	if v, ok := req["parameter_defaults"]; ok {
		if arr, ok := v.([]interface{}); ok {
			parameterDefaults = arr
		}
	}
...
func (h *AgentHandler) HandleListArtifacts(w http.ResponseWriter, r *http.Request) {
	...
	if isPlatformAdmin(r) {
		out, err = h.queryArtifacts(`SELECT id, name, display_name, source_type, path, format, task_type, architecture, size_label, quantization, default_context_length, estimated_vram_bytes, required_gpu_count, capability_set_json, default_test_mode, tenant_id, created_at, updated_at FROM model_artifacts ORDER BY name`)
	} else {
		out, err = h.queryArtifacts(`SELECT id, name, display_name, source_type, path, format, task_type, architecture, size_label, quantization, default_context_length, estimated_vram_bytes, required_gpu_count, capability_set_json, default_test_mode, tenant_id, created_at, updated_at FROM model_artifacts WHERE tenant_id = ? ORDER BY name`, tid)
	}
	if err != nil {
...
func (h *AgentHandler) HandleCreateArtifact(w http.ResponseWriter, r *http.Request) {
	...
	capabilitySet := artifactCapabilitySet(req)
	_, err := h.DB.Exec(`INSERT INTO model_artifacts (id, name, display_name, source_type, path, format, task_type, architecture, size_label, quantization, default_context_length, estimated_vram_bytes, required_gpu_count, capability_set_json, default_test_mode, tenant_id, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
		...
		jsonString(capabilitySet), strVal(req, "default_test_mode", "auto"), tid, now, now,
	)
	if err != nil {
...
func (h *AgentHandler) HandlePatchArtifact(w http.ResponseWriter, r *http.Request) {
	...
	capabilitySet := mapFromAny(existing["capability_set"])
	if v, ok := req["capabilities"]; ok {
		...
		if rawSources != nil {
			if sm, ok4 := rawSources.(map[string]interface{}); ok4 {
				...
				capabilitySet["capability_sources"] = normalizedSources
			}
		}
	} else if v, ok := req["capability_sources"]; ok {
		// Allow updating sources independently.
		capabilitySet["capability_sources"] = v
	}
	...
	if v, ok := req["parameter_defaults"]; ok {
		capabilitySet["parameter_defaults"] = v
	}
	if _, ok := req["capabilities"]; ok {
		sets = append(sets, "capability_set_json = ?")
		args = append(args, jsonString(capabilitySet))
	} else if _, ok := req["capability_sources"]; ok {
		sets = append(sets, "capability_set_json = ?")
		args = append(args, jsonString(capabilitySet))
	} else if _, ok := req["parameter_defaults"]; ok {
		sets = append(sets, "capability_set_json = ?")
		args = append(args, jsonString(capabilitySet))
	}
	args = append(args, id)
...
func (h *AgentHandler) getArtifactJSON(id string) map[string]interface{} {
	row := h.DB.QueryRow(`SELECT id, name, display_name, source_type, path, format, task_type, architecture, size_label, quantization, default_context_length, estimated_vram_bytes, required_gpu_count, capability_set_json, default_test_mode, tenant_id, created_at, updated_at FROM model_artifacts WHERE id = ?`, id)
	var rid, name, dn, st, path, frmt, tt, arch, sl, quant, capabilitySetJSON, testMode, tid, ca, ua string
	var ctxLen, gpuCount int
	var vram int64
	if err := row.Scan(&rid, &name, &dn, &st, &path, &frmt, &tt, &arch, &sl, &quant, &ctxLen, &vram, &gpuCount, &capabilitySetJSON, &testMode, &tid, &ca, &ua); err != nil {
		return nil
	}
	capabilitySet := mapFromAny(capabilitySetJSON)
	return map[string]interface{}{"id": rid, "name": name, "display_name": dn, "source_type": st, "path": path, "format": frmt, "task_type": tt, "architecture": arch, "size_label": sl, "quantization": quant, "default_context_length": ctxLen, "estimated_vram_bytes": vram, "required_gpu_count": gpuCount, "capability_set": capabilitySet, "capabilities": nilToDefault(capabilitySet["capabilities"], []interface{}{}), "capability_sources": nilToDefault(capabilitySet["capability_sources"], map[string]interface{}{}), "default_test_mode": testMode, "parameter_defaults": nilToDefault(capabilitySet["parameter_defaults"], []interface{}{}), "tenant_id": tid, "created_at": ca, "updated_at": ua, "locations": h.listModelLocations(id)}
}
...
func (h *AgentHandler) queryArtifacts(query string, args ...interface{}) ([]map[string]interface{}, error) {
	...
	for rows.Next() {
		...
		var ctxLen, gpuCount int
		var vram int64
		if err := rows.Scan(&rid, &name, &dn, &st, &path, &frmt, &tt, &arch, &sl, &quant, &ctxLen, &vram, &gpuCount, &capabilitySetJSON, &testMode, &tid, &ca, &ua); err != nil {
			continue
		}
		capabilitySet := mapFromAny(capabilitySetJSON)
...
func (h *AgentHandler) HandleDiscoverArtifact(w http.ResponseWriter, r *http.Request) {
	...
	capabilitySet := artifactCapabilitySet(req)
	_, err := tx.Exec(`INSERT INTO model_artifacts (id, name, display_name, source_type, path, format, task_type, architecture, size_label, quantization, default_context_length, estimated_vram_bytes, required_gpu_count, capability_set_json, default_test_mode, tenant_id, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
		id, name, strVal(req, "display_name", name), strVal(req, "source_type", "local_path"),
