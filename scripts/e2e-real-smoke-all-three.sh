...
smoke_vllm() {
  ...
  local dep_resp; dep_resp=$(api_post "deployments" "{\"name\":\"$PREFIX-vllm\",\"display_name\":\"vLLM Smoke\",\"model_artifact_id\":\"$HF_ART\",\"node_backend_runtime_id\":\"$NODE_ID:$rt\",\"placement_json\":{\"node_id\":\"$NODE_ID\",\"accelerator_ids\":[]},\"service_json\":{\"host_port\":8191,\"container_port\":8022,\"app_port\":8022},\"parameters_json\":{\"served_model_name\":\"e2e-vllm-smoke\",\"gpu_memory_utilization\":0.85,\"max_model_len\":4096,\"tensor_parallel_size\":1}}")
...
smoke_sglang() {
  ...
  api_post "nodes/$NODE_ID/backend-runtimes/$NODE_ID:$rt/check-request" '{}' >/dev/null || true
  local dep_resp; dep_resp=$(api_post "deployments" "{\"name\":\"$PREFIX-sglang\",\"display_name\":\"SGLang Smoke\",\"model_artifact_id\":\"$HF_ART\",\"node_backend_runtime_id\":\"$NODE_ID:$rt\",\"placement_json\":{\"node_id\":\"$NODE_ID\",\"accelerator_ids\":[]},\"service_json\":{\"host_port\":8194,\"container_port\":31000,\"app_port\":31000},\"parameters_json\":{\"served_model_name\":\"e2e-sglang-smoke\",\"tp\":1}}")
  local dep_id; dep_id=$(echo "$dep_resp" | json_field id)
